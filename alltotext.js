...
for(var j=0;j<num.length;j++){
    for(var x=0;x<school.length;x++){
        url='http://gaokao.chsi.com.cn/zsgs/zzxblqzgmd--method-listStu,ssdm-'+num[j]+',yxdm-'+school[x]+',year-2017,zslx=1.dhtml';
       // console.log(url);
        fetchPage(url);
    }
...
function savedContent($, detail) {
	...
	fs.appendFile('./data/' + name +'('+ diqu +')'+ '.txt', detail, 'utf-8', function (err) {
			if (err) {
				console.log(err);
            }
            else{
